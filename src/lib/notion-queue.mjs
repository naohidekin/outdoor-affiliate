...
export async function markPosted(
  pageId,
  {
    statusField = "ステータス",
    statusValue = "posted",
    postedAtField = null,   // null = 書き込まない（DB にプロパティが存在する場合のみ指定）
    postUrlField = null,    // null = 書き込まない
    postUrl = null,
  } = {},
  token
) {
  ...
  if (postedAtField) {
    properties[postedAtField] = { date: { start: new Date().toISOString() } };
  }
  if (postUrlField && postUrl) {
    properties[postUrlField] = { url: postUrl };
  }
  const res = await notionRequest(
    "PATCH",
    `/v1/pages/${pageId}`,
