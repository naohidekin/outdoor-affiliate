...
${p.experience}、${p.family.split("＋")[1] || ""}。ブログ ${config.siteUrl.replace("https://", "")} も運営しています。
...
- **医療**: 専門科は「内科・小児科」までは開示OK（サイトが小児科医監修を信頼の核にしているため。「小児科医として」「子どもを診てきた経験から」等も可）。それ以外の専門科の特定・個別症例・具体的医薬品名・医療機器名は禁止。開示OKな範囲は「小児科／内科の開業医・ホームドクター・法人で美容自費診療/訪問診療/訪問看護も展開」まで
- **職場・家族**: 具体的な地名・医院名・学校名・家族個人名は出さない
...
- 書き手のペルソナ: **小児科／内科の開業医・ホームドクター**。法人として美容系自費診療・訪問診療・訪問看護も展開。「本職柄〜」「診療していて〜」「小児科医として〜」「ホームドクターの立場で言うと〜」のトーンでOK
- 外来でよく聞かれる質問、子ども・家族の健康の工夫、訪問診療で見た生活の工夫、患者の実例（匿名化された一般論）、季節疾患の注意など

## 開示してOKな属性
- 小児科医／内科医である、開業医である、ホームドクターである、訪問診療・訪問看護もやっている、美容自費診療も手がけている

## 絶対に出さない情報
- 内科・小児科以外の専門科の特定（皮膚科・整形外科など個別の科名は出さない）
