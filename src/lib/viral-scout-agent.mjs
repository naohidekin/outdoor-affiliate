 ...
 *
 * CLI からは scripts/viral-scout-agent.js 経由で呼び出される。
 * Next.js API route からは直接 import して runViralScout(opts) を呼べる。
 *
 * 必要な環境変数:
...
/**
 * Viral Scout を実行する公開関数。CLI と Next.js API から共用。
 * @param {object} opts - { axis, dryRun, count, minScore, days }
 * @returns {Promise<{scouted: number, generated: number, skipped: boolean, reason?: string}>}
 */
async function runViralScout(opts) {
  ...
  if (ks.killed) {
    ...
    return { scouted: 0, generated: 0, skipped: true, reason: ks.reason };
  }
  ...
  if (scoutedPosts.length === 0) {
    ...
    return { scouted: 0, generated: 0, skipped: false };
  }
  ...
  if (opts.dryRun) {
    console.log("\n[DRY RUN] ファイル保存をスキップ");
    return { scouted: scoutedPosts.length, generated: finalPosts.length, skipped: false };
  }
  ...
  // buzz-first-lines.json 更新（read-only fs では書き込みスキップ）
  try {
    ...
    if (err && (err.code === "EROFS" || err.code === "EACCES")) {
      console.warn(`[viral-scout] buzz-first-lines.json 書き込みスキップ（read-only fs）`);
    } else {
      throw err;
    }
  }
...
// === 公開API ===
export { runViralScout, parseArgs };
