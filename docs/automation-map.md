...
13-補足（2026-08-23 訂正）. **「itemCode は使えない」は一般化しすぎだった**
    落ちた原因は urlCode を itemCode として渡していたことで、両者は別物。
    **正しい itemCode を持っていれば直引きは有効**であり、方式自体は正しい。
    キーワード検索で一度解決した itemCode を products.json に保存すれば、
    以降は直引きに切り替えられる（scripts/resolve-rakuten-itemcodes.mjs）。
    392件をキーワード検索で舐めると1秒制限で約6分半かかり、毎回取り違えの
    リスクも抱える。「使えない方法」として封印すると、この最適化を永久に失う。
...
## 2026-08-23: 楽天APIの全断と、自分のコードにあったレート制限違反

朝から全リクエストが `API Configuration not found` を返す状態が続いた。
仮説を4つ立てて4つとも外している（アプリIDとアクセスキーの対応ズレ／
IP制限／エンドポイント違い／Origin・Refererヘッダ）。切り分け用に
`scripts/check-rakuten-api.mjs` を用意し、2エンドポイント×キー有無×
ヘッダ有無の5通りを試したが全滅した。アプリ側の設定（楽天市場API有効・
IP登録）も画面で確認済みで、原因は特定できていない。

その過程で楽天のFAQに当たり、**「1つのapplication_idにつき、1秒に1回
以下のリクエストとしてください」**という明文の制限を確認した。
そこで自分のコードを見たら、破っていた。

### 落とし穴（18番目）

18. **待機の「置き場所」を間違えると、間隔の値が正しくても制限を破る**。
    `src/lib/product-lookup.mjs` には
    `await new Promise(r => setTimeout(r, 1200))` が確かに書かれていて、
    コメントも「楽天APIレート制限対策（1リクエスト/秒）」だった。
    ところが置かれていたのが**ループ本体の最後**で、途中の `continue`
    （検索0件 / 正式名で重複）を通ると待機ごと飛ばされていた。
    grepで間隔の数値だけ見ても見つからない類の違反で、
    「1200と書いてあるから守っている」と読める分むしろ危ない。
    **待機はリクエストの直後に置く**。ループの最後ではなく。

    加えて `audit-prices.mjs` と `verify-links.mjs` には、1商品につき
    2回投げる箇所（商品名で検索 → 外したらURLコードで再検索）が
    あり、その間隔が `sleep(700)` だった。360商品規模で回るので
    1回の実行で数百回の違反になる。この日は監査を4回流している。
    いずれも1200msに直した。

因果は証明できていない。ただし規定違反は事実で、application_id の
利用停止は起こりうる。原因が別だったとしても直すべきものなので、
まず自分の側を直した。**外部APIが不通になったとき、相手を疑う前に
自分が規約を守っているかを見る**。
