...
サイト側の実装・型確認・購入クリックの回帰テスト・本番ビルドを確認。専用プロジェクト接続と本番でのイベント受信確認は未完了。GA4と独自クリックビーコンは継続。
...
設定がなければSDKを読み込まず、送信しない。プレビュー・localhostも対象外。本番ドメインのトップ、記事、カテゴリ、選び方ガイドだけを対象にする。
