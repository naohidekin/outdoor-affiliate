...
// ─── 医学ガード ──────────────────────────────────────
//
// 記事は自動生成・自動公開されるが、公開前チェックは長らく文字数とFAQ数
// だけだった。その結果、暖房記事が一酸化炭素を11回書きながらやけどに
// 一度も触れない、虫よけ記事でディートの下限月齢が3種類に分裂する、
// といったことが起きていた（2026-08-26 の医師レビューで判明）。
//
// 生成の段階で釘を刺しておく。守られなかった場合は
// article-publisher-agent.js のゲートが公開を止める。
function loadMedicalGuard() {
  let standard = "";
  try {
    standard = fs.readFileSync(
      path.join(PROJECT_ROOT, "docs/repellent-age-standard.md"),
      "utf-8"
    );
  } catch {
    // 無ければ無しで続ける
  ...
  return `
【医学・安全に関するルール（最優先。他のどのルールより優先する）】

このサイトは「現役小児科開業医」を信頼の核にしている。医学的に不正確な
記述は、記事1本の問題では済まない。次を必ず守ること。

1. 断定しない。熱傷の深さ・症状の出方・効果の程度は条件で変わる。
   「深いやけどになります」ではなく「深いやけどになりやすくなります」。
2. 機序を書く。「子どもは体格が小さいから危ない」のような説明は誤り。
   例: 小児は体重あたりの呼吸量が多いため、同じ濃度の一酸化炭素に
   さらされると体重あたりでは速く取り込み、症状が早く現れる可能性がある。
3. 別々の理由を1文でつながない。体温調節（体表面積・調節機能の未熟さ）と
   脱水（必要水分量の多さ・水分予備量の少なさ）は別の機序。
4. 応急処置に条件を付ける。「やけどは流水15〜20分」は小範囲の話で、
   乳幼児や広範囲では冷やしすぎによる低体温の危険がある。
5. 効能効果を断定しない（薬機法・医療法）。「安全です」「効果があります」
   「予防できます」「治ります」は使わない。
6. 個別の医療相談に答えない。医薬品名・医療機器名の具体は出さない。
   受診の目安は書いてよい。
7. 一酸化炭素・やけど・熱中症・低体温・誤飲・虫よけ成分・乳幼児に
   触れる記事は、医師の確認を前提とする。断定を避け、判断が要る箇所は
   「受診してください」に寄せること。
${standard ? "\n【虫よけの年齢基準（この文書が唯一の正。ここから外れないこと）】\n" + standard : ""}
`;
}
...
const SCORING_CRITERIA = [
  ...
  "医学的正確性（断定を避ける・機序を書く・応急処置に条件を付ける・薬機法/医療法に触れない）",
...
AIO（AI検索最適化）ルール:
- **本文にH1（# 見出し）は絶対に書かない**。タイトルはページ側で自動的にh1として表示されるため、
  本文冒頭にタイトルを繰り返すとh1が二重になりSEO違反になる。本文の見出しはH2（##）から始めること
