...
async function processDb(dbConfig, { dryRun }, token) {
  ...
  for (const page of pages) {
    ...
    // 投稿タイプ: "引用" なら引用ポスト、それ以外は通常リプライ（手動対応のためスキップ）
    const postTypeName = props["投稿タイプ"]?.select?.name ?? null;
    const isQuote = postTypeName === "引用";

    if (type === "reply" && !isQuote) {
      console.log(`  → 通常リプライはスキップ (手動対応)`);
      skipped++;
      continue;
    }
    ...
    // 自己返信 (selfReply) があれば本文ポストへの返信として投稿
    if (tweetId && type === "post") {
      const selfReplyText = extractText(props["自己返信"]);
      if (selfReplyText?.trim()) {
        try {
          await new Promise((r) => setTimeout(r, 1500));
          await xClient.v2.tweet(selfReplyText, {
            reply: { in_reply_to_tweet_id: tweetId },
          });
          console.log(`    ✓ 自己返信 投稿完了`);
        } catch (err) {
          const detail = err.data ? JSON.stringify(err.data) : err.message;
          console.warn(`    ⚠ 自己返信 投稿失敗 (${err.code ?? "?"}): ${detail}`);
          // 自己返信失敗でも本体投稿は成功扱い
        }
      }
    }
