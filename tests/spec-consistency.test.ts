...
test("同シリーズでサイズと価格・重量が逆転していない", () => {
  ...
  //
  // 2026-09-03: アメニティドームS の ¥44,000 を解消して空にした。
  // 削除した重複レコード tent-sp-amenity-dome-m の価格が混ざったものだった。
  // 楽天APIで SDE-002 の実売11件を引いて中央値 ¥33,440 に置き換えた
  // （最安 ¥19,998 / アイボリー ¥35,798 前後）。S < M < L の順に戻っている。
  const KNOWN: string[] = [];
