...
test("同シリーズでサイズと価格・重量が逆転していない", () => {
  ...
  // 値が混ざった痕跡がここにも出ている。
  //
  // 2026-09-02: その重複レコードを削除した（Amazon の ASIN が tent-002 と
  // 同一だった）。ただし**S の価格は直っていない**ので逆転は残る。
  // Amazon では アメニティドーム S アイボリー（B0BCQ6DJTB）が ¥30,101 で、
  // ¥44,000 は高すぎる可能性が高い。実売を確認して直したらここから消す。
